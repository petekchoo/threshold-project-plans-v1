...
export function ProjectActivityList({ projectId, data, onNew, onEdit, readOnly = false }: { projectId: string; data: AppData; onNew: () => void; onEdit: (activity: Activity) => void; readOnly?: boolean }) {
  const filters = useActivityFilters(data, '', projectId);
  return <section className="project-activity-list">
    <div className="section-head"><div><p className="eyebrow">Project work</p><h2>Activities</h2></div>{!readOnly&&<button className="secondary-btn project-add-activity" onClick={onNew}><Icon name="plus"/>Add activity</button>}</div>
    <ActivityFilterBar data={data} filters={filters}/>
    {filters.rows.length ? filters.rows.map((activity) => readOnly ? <div className="project-activity-row" key={activity.id}>
    ...
    </div> : <button type="button" onClick={() => onEdit(activity)} className="project-activity-row" key={activity.id}>
      <div><strong>{activity.name}</strong><small>{activity.activity_types?.name || 'Activity'}{activity.archived_at ? ' · Archived' : ''}</small></div>
      <AvatarList owners={activity.activity_owners?.map((owner) => owner.team_members) || []}/><Pill value={activity.status}/><span>{date(activity.start_date)} — {date(activity.due_date)}</span>
