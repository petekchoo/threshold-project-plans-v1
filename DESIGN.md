...
| PRJ-06 | Direct project-date edits preview and atomically move the complete or remaining schedule while preserving activity formation and completed history | Pure planner in `lib/planning/project-reschedule.ts`; delivery plan in `plans/PRJ-06-project-rescheduling.md` | No atomic project-rescheduling function | Partial | Pure final-state planning and unit coverage implement end anchoring, shift/expand/compress outcomes, fixed completed and archived work, both dependency types in both project directions, project-relative timing, date exceptions, invalid calendar data, and complete structured conflicts. Authoritative database planning, atomic persistence, preview UI, transaction tests, and core-journey coverage remain. |
