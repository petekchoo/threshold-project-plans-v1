...
select is(public.save_project_template('{"id":"54000000-0000-0000-0000-000000000002","name":"Dependent branch","project_type_id":"52000000-0000-0000-0000-000000000001"}'::jsonb),'54000000-0000-0000-0000-000000000002'::uuid,'dependent-branch template saves');
select lives_ok($$select public.save_project_template_activity('{"id":"55000000-0000-0000-0000-000000000010","template_id":"54000000-0000-0000-0000-000000000002","activity_type_id":"53000000-0000-0000-0000-000000000001","name":"Activity C","duration_days":2,"sort_order":0,"rules":[]}'::jsonb)$$,'unanchored prerequisite C saves');
select lives_ok($$select public.save_project_template_activity('{"id":"55000000-0000-0000-0000-000000000011","template_id":"54000000-0000-0000-0000-000000000002","activity_type_id":"53000000-0000-0000-0000-000000000001","name":"Activity D","duration_days":2,"sort_order":1,"rules":[{"schedule_rule":"start_after_activity_finish","offset_days":1,"relative_activity_id":"55000000-0000-0000-0000-000000000010"}]}'::jsonb)$$,'dependent branch D saves');
select lives_ok($$select public.save_project_template_activity('{"id":"55000000-0000-0000-0000-000000000012","template_id":"54000000-0000-0000-0000-000000000002","activity_type_id":"53000000-0000-0000-0000-000000000001","name":"Activity B","duration_days":2,"sort_order":2,"rules":[{"schedule_rule":"start_after_activity_finish","offset_days":0,"relative_activity_id":"55000000-0000-0000-0000-000000000010"}]}'::jsonb)$$,'anchored-chain activity B saves');
select lives_ok($$select public.save_project_template_activity('{"id":"55000000-0000-0000-0000-000000000013","template_id":"54000000-0000-0000-0000-000000000002","activity_type_id":"53000000-0000-0000-0000-000000000001","name":"Activity A","duration_days":2,"sort_order":3,"rules":[{"schedule_rule":"finish_before_project_end","offset_days":4},{"schedule_rule":"start_after_activity_finish","offset_days":0,"relative_activity_id":"55000000-0000-0000-0000-000000000012"}]}'::jsonb)$$,'project-end anchor A saves');
select is(public.preview_project_template('54000000-0000-0000-0000-000000000002','2026-09-20')#>>'{activities,1,start_date}','2026-09-15','dependent branch starts at its earliest valid offset');

create temporary table branch_project as select public.create_project_from_template('54000000-0000-0000-0000-000000000002','Dependent branch project','2026-09-20') id;
select is((select start_date::text||'|'||due_date::text from public.activities where project_id=(select id from branch_project) and name='Activity D'),'2026-09-15|2026-09-16','materialization preserves the derived dependent-branch dates');
select is((select d.offset_days from public.activity_dependencies d join public.activities a on a.id=d.activity_id where a.project_id=(select id from branch_project) and a.name='Activity D'),1,'materialization preserves the dependency offset');
select is((select start_date::text from public.projects where id=(select id from branch_project)),'2026-09-13','materialization uses the earliest derived activity as project start');
...
select is((select count(*)::integer from public.projects),2,'failed materialization creates no partial project');
