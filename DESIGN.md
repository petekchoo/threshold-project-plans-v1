...
- A completed prerequisite satisfies its dependency regardless of its retained planned dates. This rule applies consistently to activity editing, dependency validation, propagation, and project rescheduling. A completed dependent whose prerequisite remains incomplete represents inconsistent state and must be resolved rather than silently rescheduled.
...
| PRJ-06 | Direct project-date edits preview and atomically move the complete or remaining schedule while preserving activity formation and completed history | Planned in `plans/PRJ-06-project-rescheduling.md` | No atomic project-rescheduling function | Planned | Confirmed behavior covers project-end anchoring, shift/expand/compress outcomes, completed and archived activities, and manual handling of cross-project constraints. Implementation and automated unit, transaction, and core-journey coverage remain. |
...
| ID | Capability | Primary implementation | Database / persistence | Coverage | Verification / known gap |
| --- | --- | --- | --- | --- | --- |
| DEP-01 | Filter non-archived activities by project, search by Activity Name, then stage, save, configure, follow, and remove prerequisites | `components/activities/dependency-picker.tsx`; `components/activities/activity-form.tsx`; `components/activities/activity-detail.tsx` | `activity_dependencies`; authoritative `save_activity` in migration `202608300003` | Partial | Project-filtered Activity Name search and staged single-selection Save/Cancel are implemented in a modal. Selected prerequisites have explicit constraint and Remove controls. Dependents are not shown on activity detail. |
| DEP-02 | Can't-start-until-completed and can't-finish-until-completed prerequisite constraints; completed prerequisites satisfy their relationships | `components/activities/activity-form.tsx` (`submit`, constraint selector) | `dependency_type`; `activity_dependencies` | Partial | Direct dependent-activity adjustments exist. Completed-prerequisite satisfaction is confirmed and planned with PRJ-06. Database writes do not enforce FTS/FTF date validity, and prerequisite edits do not inspect downstream activities. |
| DEP-03 | Preview and confirm dependency-driven date changes while preserving valid scheduling space | `components/activities/activity-form.tsx` (`submit`, neutral confirmation copy) | Authoritative `save_activity` performs atomic activity/owner/link/dependency save | Partial | Handles direct changes to the edited dependent. PRJ-06 will apply completed-prerequisite satisfaction consistently and block completed-dependent/incomplete-prerequisite inconsistencies. Multi-level propagation, downstream impact previews, reverse prerequisite proposals, and the specified update/remove resolution are not implemented. |
...
| QA-03 | Automated unit tests for scheduling calculations | Vitest; `lib/planning/scheduling.test.ts` | Partial | Project-timing date arithmetic and neutral boundary messages are covered. Add both dependency types and project-edit scenarios. |
...
| 2026-09-04 | Treat a completed prerequisite as satisfying its relationship everywhere; require completed-dependent/incomplete-prerequisite inconsistencies to be resolved explicitly. | Prevents historical planned dates from creating artificial conflicts while retaining visibility into logically inconsistent completion state. | Confirmed |
...
- 2026-09-04: Confirmed global completed-prerequisite semantics and the separate status-change path for reopening completed work before a whole-schedule move.
