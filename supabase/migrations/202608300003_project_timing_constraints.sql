alter table public.activities
  add column if not exists project_timing_rule text,
  add column if not exists project_timing_boundary text,
  add column if not exists project_timing_offset_days integer;
...
alter table public.activities
  ...
  add constraint activities_project_timing_complete check (
    ...
    or (
      ...
      and project_timing_boundary in ('start', 'end')
      and project_timing_offset_days >= 0
      and (project_timing_rule <> 'post_project_deadline' or project_timing_boundary = 'end')
    )
  );
...
before insert or update of project_id, due_date, project_timing_rule, project_timing_boundary, project_timing_offset_days
...
begin
  ...
  for v_activity in select * from public.activities where project_id = new.id and archived_at is null and project_timing_rule is not null loop
...
create trigger validate_project_activity_timing
before update of start_date, end_date on public.projects
for each row execute function public.validate_project_activity_timing();
...
begin
  ...
  if nullif(p_activity->>'id', '') is null then
    insert into public.activities (
      ...
      (p_activity->>'project_id')::uuid, nullif(p_activity->>'activity_type_id', '')::uuid, trim(p_activity->>'name'),
      (p_activity->>'status')::public.activity_status, (p_activity->>'priority')::public.activity_priority,
      (p_activity->>'start_date')::date, (p_activity->>'due_date')::date, coalesce(p_activity->>'notes', ''),
    ...
    update public.activities set
      project_id = (p_activity->>'project_id')::uuid, activity_type_id = nullif(p_activity->>'activity_type_id', '')::uuid,
      name = trim(p_activity->>'name'), status = (p_activity->>'status')::public.activity_status,
      priority = (p_activity->>'priority')::public.activity_priority, start_date = (p_activity->>'start_date')::date,
