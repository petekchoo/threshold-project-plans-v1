...
export function TemplateDetail({template,data,refresh,setToast}:{template:ProjectTemplate;data:AppData;refresh:()=>Promise<void>;setToast:(value:string)=>void}){
  ...
  return <>
    ...
    <div className="table-card template-table"><table><thead><tr><th>Activity</th><th>Type</th><th>Rule</th><th>Offset</th><th>Duration</th><th>Reference</th><th aria-label="Actions"/></tr></thead><tbody>{template.activities.map(activity=>{const reference=template.activities.find(row=>row.id===activity.relative_activity_id);return <tr key={activity.id}><td><button className="link-button" onClick={()=>setEditingActivity(activity)}><strong>{activity.name}</strong></button></td><td>{activity.activity_types?.name}</td><td>{ruleLabel[activity.schedule_rule]}</td><td>{days(activity.offset_days)}</td><td>{days(activity.duration_days)}</td><td>{reference?.name||'Project end'}</td><td><button className="text-btn" onClick={()=>archiveActivity(activity)}>Archive</button></td></tr>})}</tbody></table>{!template.activities.length&&<div className="empty-state"><h3>No activities yet</h3><p>Add an activity anchored to project end to begin the reusable schedule.</p><button className="secondary-btn" onClick={()=>setEditingActivity(null)}>＋ Add activity</button></div>}</div>
