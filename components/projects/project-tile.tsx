...
export function ProjectTile({ project, activities }: { project: Project; activities: Activity[] }) {
  ...
  const completed = rows.filter(activity => activity.status === 'completed').length;
  ...
  return <Link href={`/projects/${project.id}`} className="project-tile">
    <div className="card-title project-tile-header"><div><small>{project.project_types?.name || 'Project'}</small><h3>{project.name}</h3></div><Pill value={project.status}/></div>
    <div className="project-tile-body">
      ...
      <div className="progress"><div><span>Activities</span><strong>{completed} / {rows.length}</strong></div><div className="project-card-track project-completion-track" role="img" aria-label={`${completed} of ${rows.length} activities completed, ${completion} percent`}><i style={{ width: `${completion}%` }}/></div></div>
      <div className="project-card-signals"><span className={overdue ? 'has-alert' : ''}><b>{overdue}</b> overdue</span><span className={blocked ? 'has-alert' : ''}><b>{blocked}</b> blocked</span></div>
    </div>
    <div className="next project-tile-footer"><span>{next ? `Next · ${date(next.due_date)}` : 'Next'}</span><strong>{next?.name || 'No open activities'}</strong></div>
  </Link>;
}
