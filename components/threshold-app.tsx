...
export default function ThresholdApp({view,id}:{view:Kind;id?:string}){
 ...
 const selectedProject=[...data.projects,...(data.archivedProjects||[])].find(p=>p.id===id),selectedActivity=[...data.activities,...(data.archivedActivities||[])].find(a=>a.id===id),selectedTemplate=data.templates.find(template=>template.id===id);
