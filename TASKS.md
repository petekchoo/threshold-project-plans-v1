...
- `TPL-01` / `TPL-02` project templates and project-from-template creation are ready to merge through GitHub PR #27 from `feature/project-templates`. Separate undated templates, deterministic rule/offset/duration scheduling, graph validation, Templates navigation, derived readiness, inline referenced-activity creation, searchable project creation, preview, and atomic materialization are implemented. Final `pnpm verify` passes with 64 tests, the production build passes, local schema lint reports no errors, and `pnpm db:test` passes 31 assertions including the 12-case template transaction suite. Authorized Docker-backed browser validation passed the complete authoring and materialization journey on desktop and at 390×844 mobile, including inline reference creation, exact preview dates, generated Draft/Not Started records, mobile navigation/editor containment, 390px document width, and no console errors; physical-iPhone validation against the linked backend also confirmed the released mobile flow after enabling the LAN origin. Migration `202609060001_project_templates.sql` is applied to the linked Supabase backend, whose post-apply status and schema lint are clean. Development and QA documentation now distinguish desktop, emulated mobile, and physical-phone setup, including LAN origin allowance and origin-specific authentication. GitHub Verify run 64 and Vercel passed on documentation-complete head `3cdbd62`; the final handoff-only commit must clear the same gates before merge. This branch carries the intentional post-PR #26 local handoff commits beyond `origin/main`.
