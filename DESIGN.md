...
| QA-03 | Automated unit tests for scheduling calculations | Vitest; `lib/planning/scheduling.test.ts`; `lib/planning/project-schedule.test.ts`; `lib/planning/project-reschedule.test.ts` | Partial | Project timing, project rescheduling, fixed daily project-schedule spacing and Monday marks, both dependency types, completed chronology, cross-project direction, exceptions, archive behavior, and calendar boundaries are covered. Add unit coverage for later scheduling features as they are implemented; transaction coverage remains under QA-04. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#technical-direction](#technical-direction); Verification: `pnpm test` |
