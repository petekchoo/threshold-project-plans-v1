...
| PRJ-06 | Direct project-date edits preview and atomically move the complete or remaining schedule while preserving activity formation and completed history | Binding detail and acceptance matrix in `plans/PRJ-06-project-rescheduling.md`; pure planner in `lib/planning/project-reschedule.ts` | No atomic project-rescheduling function | Partial | The linked plan maps 34 executable PRJ-06 planner tests plus 12 shared timing tests across end anchoring, shift/expand/compress outcomes, fixed completed and archived work, both dependency types in both project directions, project-relative timing, date exceptions, invalid calendar data, and complete structured conflicts. Authoritative database planning, atomic persistence, preview UI, transaction tests, and core-journey coverage remain. |
