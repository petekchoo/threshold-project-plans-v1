...
- Activities may exist outside their project's manually entered date window. A conforming post-project completion window is explicit scheduling intent and does not also trigger the generic outside-project warning.
...
- `components/threshold-app.tsx`: Current application shell, views, forms, date logic, and client-side validation.
...
| --- | --- | --- | --- | --- | --- |
| AUTH-01 | Sign up, sign in, sign out, and redirect unauthenticated users | `app/sign-in/page.tsx`; `ThresholdApp`; `Sidebar`; `MobileNav` | Supabase Auth; `profiles`; `handle_new_user` | Implemented | Manual UI; no automated auth tests. |
| AUTH-02 | Universal authenticated access to shared MVP data | `loadData`; `lib/supabase.ts` | RLS policies named `authenticated shared access` in `202608290001_initial_schema.sql` | Implemented | Database policy inspection; role-based access intentionally deferred. |
| DATA-01 | Load shared planning data and refresh on database changes | `loadData`; `ThresholdApp.refresh`; `threshold-live` realtime channel | Projects, activities, types, owners, links, and dependencies tables | Implemented | Manual UI. Realtime behavior has no automated integration test. |
| DATA-02 | Recover once from a transient JWT/session timing error without a refresh loop | `isRetryableAuthError`; `ThresholdApp.refresh`; `refreshInFlight` | Supabase session refresh | Implemented | Type check and manual reload. Retry is bounded to one session refresh and one data retry. |
...
| NAV-01 | Desktop sidebar navigation and account controls | `Sidebar`; routes under `app/` | Profile name from `profiles` | Implemented | Manual desktop UI. |
| NAV-02 | Mobile bottom navigation with administration/account overflow | `MobileNav`; `app/extended.css` | Supabase sign-out | Implemented | Manual responsive UI; breakpoint-specific keyboard testing remains. |
...
| ID | Capability | Primary implementation | Database / persistence | Coverage | Verification / known gap |
| --- | --- | --- | --- | --- | --- |
| OVR-01 | Cross-project overview timeline with project bars | `Overview`; `timelinePosition`; `/` route | Project dates and statuses | Partial | Actual dates position bars. Adaptive day/week/month/quarter headers and overview sort/filter controls are incomplete. |
...
| PRJ-05 | Project date changes respect activity project-timing constraints | `ProjectForm`; `projectTimingConflict` | `validate_project_activity_timing` trigger in `202608300003_project_timing_constraints.sql` | Partial | Client feedback names dates and differences and the database rejects invalid direct writes. Database fallback errors do not yet provide the same full context. |
...
| DEP-05 | Activity-level dependency timeline shows project, selected activity, prerequisites, and dependents with filters | `ActivityDetail` focus timeline | Dependency data | Partial | Project and selected activity render. Relationship lanes, dependents, direction labels, and All/Prerequisites/Dependents filters are not implemented. |
| DEP-06 | Advance project deadline: finish N calendar days before project start or end | `ActivityForm` Project timing controls; `projectTimingDeadline`; `projectTimingConflict` | Activity timing columns; `validate_activity_project_timing`; updated `save_activity` | Implemented | Type check, manual UI, and live schema verification. No automated boundary tests. |
| DEP-07 | Post-project completion window: finish within N calendar days after project end; start may occur earlier | `ActivityForm` Project timing controls; `projectTimingDeadline`; `projectTimingConflict` | Activity timing columns; validation triggers; authoritative `save_activity` | Partial | Deadline persistence/enforcement works. Current warning suppression also permits an unrelated pre-project-start exception when the deadline is valid; authorization must be limited to the post-project finish. |
| DEP-08 | Neutral, factual scheduling warnings identify dates and quantify differences | `projectTimingConflict`; dependency adjustment and outside-window copy in `ActivityForm` | Database errors provide neutral fallback messages | Partial | New timing messages follow the convention. Older archive and dependency messages need a comprehensive language review. |
| DEP-09 | Activities may exist outside project dates with an explicit exception | `ActivityForm.submit` | `allow_outside_project` | Partial | The UI obtains confirmation, but direct authenticated writes can bypass it and the post-project suppression is currently broader than the intended exception. |

### Administration, archives, responsive design, and presentation

| ID | Capability | Primary implementation | Database / persistence | Coverage | Verification / known gap |
| --- | --- | --- | --- | --- | --- |
| ADM-01 | Centrally manage team members, project types, and activity types | `Administration`; `AdminList`; `/administration` route | `team_members`; `project_types`; `activity_types`; active-name uniqueness migration | Implemented | Add, rename, and archive exist. Administration permission policy remains universal for MVP. |
| ARC-01 | Hide archived projects and activities by default and expose them through filters | `loadData`; `Projects`; `Activities` | `archived_at` fields | Partial | Archived rows appear through filters, but their detail routes resolve only active records and therefore render no detail content. |
| ARC-02 | Archive projects and their activities/links/dependencies | `ProjectDetail.archiveProject` | `archive_project` | Implemented | Confirmation exists; no restore flow. |
...
| VIS-01 | Threshold identity with warm editorial and restrained operational styling | `Sidebar`; logo assets; `app/globals.css`; `app/extended.css` | `public/threshold-logo.png`; `public/threshold-logo-white.png` | Implemented | Exact brand typography and color confirmation remain open under D-16. |
