...
export function Projects({ data, query, onNew }: { data: AppData; query: string; onNew: () => void }) {
  ...
  return <>
    ...
    <div className="filters project-filters">
      ...
      <label className="filter-check project-archive-filter"><input type="checkbox" checked={showArchived} onChange={(event) => setShowArchived(event.target.checked)}/> Show archived</label>
