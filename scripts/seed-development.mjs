...
if (testUser) {
  testUser = await requireSuccess(admin.auth.admin.updateUserById(testUser.id, {
    password,
    email_confirm: true,
    user_metadata: { full_name: 'Threshold Development Tester' },
...
insert into public.team_members (id, full_name, initials, email) values
  ('${fixtureIds.members[0]}', 'Avery Morgan', 'AM', 'avery@example.test'),
  ('${fixtureIds.members[1]}', 'Jordan Lee', 'JL', 'jordan@example.test'),
  ('${fixtureIds.members[2]}', 'Riley Chen', 'RC', 'riley@example.test');
insert into public.project_types (id, name) values
  ('${fixtureIds.projectTypes[0]}', 'Development Event'),
  ('${fixtureIds.projectTypes[1]}', 'Development Campaign');
insert into public.activity_types (id, name) values
  ('${fixtureIds.activityTypes[0]}', 'Development Planning'),
  ('${fixtureIds.activityTypes[1]}', 'Development Production'),
  ('${fixtureIds.activityTypes[2]}', 'Development Communications');

insert into public.projects (id, name, description, project_type_id, status, start_date, end_date, created_by) values
  ('${fixtureIds.projects[0]}', 'Development Gala', 'Resettable fixture for local development and browser validation.', '${fixtureIds.projectTypes[0]}', 'on_track', '${addDays(-10)}', '${addDays(30)}', '${testUser.id}'),
  ('${fixtureIds.projects[1]}', 'Development Fall Campaign', 'Future-project fixture.', '${fixtureIds.projectTypes[1]}', 'draft', '${addDays(20)}', '${addDays(60)}', '${testUser.id}'),
  ('${fixtureIds.projects[2]}', 'Development Completed Dinner', 'Completed-project fixture.', '${fixtureIds.projectTypes[0]}', 'completed', '${addDays(-60)}', '${addDays(-30)}', '${testUser.id}');
...
insert into public.activities (id, project_id, activity_type_id, name, status, priority, start_date, due_date, notes, allow_outside_project, project_timing_rule, project_timing_boundary, project_timing_offset_days, created_by) values
  ('${activities[0]}', '${fixtureIds.projects[0]}', '${fixtureIds.activityTypes[0]}', 'Confirm gala brief', 'completed', 'normal', '${addDays(-10)}', '${addDays(-8)}', 'Completed prerequisite fixture.', false, null, null, null, '${testUser.id}'),
  ('${activities[1]}', '${fixtureIds.projects[0]}', '${fixtureIds.activityTypes[0]}', 'Finalize guest requirements', 'in_progress', 'high', '${addDays(-7)}', '${addDays(2)}', 'Active owned fixture.', false, null, null, null, '${testUser.id}'),
  ('${activities[2]}', '${fixtureIds.projects[0]}', '${fixtureIds.activityTypes[1]}', 'Produce gala materials', 'not_started', 'normal', '${addDays(2)}', '${addDays(10)}', '', false, null, null, null, '${testUser.id}'),
  ('${activities[3]}', '${fixtureIds.projects[0]}', '${fixtureIds.activityTypes[2]}', 'Resolve overdue invitation list', 'blocked', 'urgent', '${addDays(-5)}', '${addDays(-1)}', 'Unassigned overdue fixture.', false, null, null, null, '${testUser.id}'),
  ('${activities[4]}', '${fixtureIds.projects[0]}', '${fixtureIds.activityTypes[2]}', 'Send post-event follow-up', 'not_started', 'normal', '${addDays(29)}', '${addDays(35)}', '', true, 'post_project_deadline', 'end', 7, '${testUser.id}');
...
insert into public.activity_links (id, activity_id, label, url, sort_order) values
  ('76000000-0000-0000-0000-000000000001', '${activities[1]}', 'Development brief', 'https://example.com/development-brief', 0);
...
update public.profiles set full_name = 'Threshold Development Tester' where id = '${testUser.id}';
commit;
`;
