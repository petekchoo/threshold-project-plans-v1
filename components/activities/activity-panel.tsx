...
export function ActivityPanel({ activity, data, onClose, onSaved }: { activity: Activity; data: AppData; onClose: () => void; onSaved: () => void }) {
  useEffect(()=>{function handleKeyDown(event:KeyboardEvent){if(event.key==='Escape'&&!document.querySelector('[data-editor-child-modal="true"]'))onClose()}document.addEventListener('keydown',handleKeyDown);return()=>document.removeEventListener('keydown',handleKeyDown)},[onClose]);
