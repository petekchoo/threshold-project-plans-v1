...
export function Projects({ data, query, onNew }: { data: AppData; query: string; onNew: () => void }) {
  ...
  return <>
    ...
    <div className="table-card"><table><thead><tr><th><button className="table-sort" type="button" aria-pressed={sort==='name'} onClick={()=>setSort('name')}>Project {sort==='name'&&<span aria-hidden="true">↑</span>}</button></th><th>Type</th><th><button className="table-sort" type="button" aria-pressed={sort==='status'} onClick={()=>setSort('status')}>Status {sort==='status'&&<span aria-hidden="true">↑</span>}</button></th><th><button className="table-sort" type="button" aria-pressed={sort==='start_date'} onClick={()=>setSort('start_date')}>Start {sort==='start_date'&&<span aria-hidden="true">↑</span>}</button></th><th><button className="table-sort" type="button" aria-pressed={sort==='end_date'} onClick={()=>setSort('end_date')}>End {sort==='end_date'&&<span aria-hidden="true">↑</span>}</button></th><th className="numeric">Progress</th><th className="numeric">Overdue</th><th>Team members</th></tr></thead><tbody>{projects.map((project) => { const overdue = data.activities.filter((activity) => activity.project_id === project.id && activity.status !== 'completed' && activity.due_date < today).length; const projectProgress=progress(project,data.activities); return <tr key={project.id}><td><Link href={`/projects/${project.id}`}><strong>{project.name}</strong></Link>{project.archived_at ? <small>Archived</small> : project.description ? <details className="project-description"><summary>Project brief</summary><p>{project.description}</p></details> : null}</td><td>{project.project_types?.name}</td><td><Pill value={project.status}/></td><td>{date(project.start_date)}</td><td>{date(project.end_date)}</td><td className="numeric"><div className="table-progress"><div className="table-progress-track" role="progressbar" aria-label={`${project.name} activity progress`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={projectProgress}><i style={{width:`${projectProgress}%`}}/></div><strong>{projectProgress}%</strong></div></td><td className="numeric">{overdue}</td><td><AvatarList owners={project.project_owners?.map((owner) => owner.team_members) || []}/></td></tr>; })}</tbody></table></div>
