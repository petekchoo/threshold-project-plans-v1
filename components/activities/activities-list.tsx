...
export function ActivitiesList({ data, query, onNew }: { data: AppData; query: string; onNew: () => void }) {
  ...
  return <>
    ...
    {rows.length ? <>
      <div className="table-card"><table><thead><tr><th><button className="table-sort" type="button" aria-pressed={filters.sort==='name'} onClick={()=>filters.setSort('name')}>Activity {filters.sort==='name'&&<span aria-hidden="true">↑</span>}</button></th><th>Project</th><th>Type</th><th>Team members</th><th><button className="table-sort" type="button" aria-pressed={filters.sort==='status'} onClick={()=>filters.setSort('status')}>Status {filters.sort==='status'&&<span aria-hidden="true">↑</span>}</button></th><th><button className="table-sort" type="button" aria-pressed={filters.sort==='priority'} onClick={()=>filters.setSort('priority')}>Priority {filters.sort==='priority'&&<span aria-hidden="true">↑</span>}</button></th><th><button className="table-sort" type="button" aria-pressed={filters.sort==='start_date'} onClick={()=>filters.setSort('start_date')}>Start {filters.sort==='start_date'&&<span aria-hidden="true">↑</span>}</button></th><th><button className="table-sort" type="button" aria-pressed={filters.sort==='due_date'} onClick={()=>filters.setSort('due_date')}>Due {filters.sort==='due_date'&&<span aria-hidden="true">↑</span>}</button></th><th>Overdue</th></tr></thead><tbody>{rows.map(activity => <tr key={activity.id}><td><Link href={`/activities/${activity.id}`}><strong>{activity.name}</strong><small>{activity.archived_at ? 'Archived' : ''}</small></Link></td><td>{activity.projects?.name}</td><td>{activity.activity_types?.name}</td><td><AvatarList owners={activity.activity_owners?.map(item => item.team_members) || []}/></td><td><Pill value={activity.status}/></td><td><span className={`priority priority-${activity.priority}`}>{activity.priority}</span></td><td>{date(activity.start_date)}</td><td>{date(activity.due_date)}</td><td>{activity.status !== 'completed' && activity.due_date < today ? 'Overdue' : '—'}</td></tr>)}</tbody></table></div>
