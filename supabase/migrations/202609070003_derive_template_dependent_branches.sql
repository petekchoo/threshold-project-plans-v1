...
begin
  ...
  for v_item in select value from jsonb_array_elements(v_plan->'activities') order by (value->>'sort_order')::integer loop
    ...
    values(v_activity_id,v_project_id,(v_item->>'activity_type_id')::uuid,v_item->>'name','not_started','normal',(v_item->>'start_date')::date,(v_item->>'due_date')::date,'',(v_item->>'due_date')::date>p_project_end,
      case when v_project_rule->>'schedule_rule'='finish_before_project_end' then 'advance_deadline' when v_project_rule->>'schedule_rule'='finish_after_project_end' then 'post_project_deadline' end,
  ...
  for v_item in select value from jsonb_array_elements(v_plan->'activities') loop
    for v_rule in select value from jsonb_array_elements(v_item->'rules') where value->>'relative_activity_id' is not null loop
      insert into public.activity_dependencies(activity_id,depends_on_activity_id,constraint_type,offset_days)
      values((v_ids->>(v_item->>'template_activity_id'))::uuid,(v_ids->>(v_rule->>'relative_activity_id'))::uuid,
        case when v_rule->>'schedule_rule'='start_after_activity_finish' then 'finish_to_start'::public.dependency_type else 'finish_to_finish'::public.dependency_type end,(v_rule->>'offset_days')::integer);
    end loop;
  end loop;
