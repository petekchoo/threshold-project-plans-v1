...
export function Overview({ data, refresh, setToast }: { data: AppData; refresh: () => void; setToast: (value: string) => void }) {
  ...
  const gridLines = scale.gridMarks.map((mark,index)=><i aria-hidden="true" className={`overview-grid-line ${index===scale.gridMarks.length-1?'is-terminal':''}`} key={mark.date} style={{left:`${mark.left}%`}}/>);
  const scaleLabel = `${range} timeline from ${scale.domainStart} through ${addDays(scale.domainEnd,-1)}, divided by ${scale.gridUnit}`;
