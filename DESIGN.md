...
The application must support desktop and mobile. Desktop provides the densest planning experience. Mobile uses a project-tile overview rather than compressing the master Gantt and supports full project/activity creation and editing with the same underlying controls adapted responsively. Mobile project tiles show project name, type, status, an endpoint-labelled project-window bar with elapsed fill and an in-window Today marker, an exactly aligned activity-completion bar, overdue and blocked activity counts, and the next upcoming activity. Exact breakpoints and individual control adaptations remain open.

- Mobile uses a non-sticky header that naturally scrolls away with the page, avoiding unstable browser visual-viewport edges. The drawer visibly and semantically identifies the current destination and supports accessible dismissal, focus trapping, and focus return.
- On small screens, primary search remains immediately available while secondary activity and project filters use compact disclosures. Every confirmed filter remains available, active filters are apparent, and users can clear them together. Desktop keeps the complete filter sets expanded.
...
- Template activities contain only a required name, activity type, one positive integer calendar-day duration, and one or more scheduling constraints with an offset and an activity reference when the constraint requires one. They do not carry status, priority, team members, notes, or links.
- Duration is an activity detail rather than part of any individual constraint. It defaults to one day, must be at least one, and uses inclusive calendar-day arithmetic: duration `1` produces a same-day activity and duration `N` finishes `N - 1` calendar days after its start. Constraint offsets remain non-negative magnitudes; zero offset uses the anchor date. Finish-after-project-end requires an offset of at least one.
- A template activity may combine constraints requiring it to finish before project end, finish after project end, start after another template activity finishes, or finish after another template activity finishes. The schedule resolver places every activity as late as possible while satisfying all constraints. The complete constraint graph must be acyclic, connected directly or transitively to project end, bounded, and jointly satisfiable regardless of display or creation order.
- The template activity **Schedule** section follows the same staged-item pattern as links and team members. It starts with no rules and a **+ Add schedule rule** action. A rule editor collects the rule and offset; choosing an activity-relative rule additionally reveals the activity selector and inline **+ Create activity** action. Saving adds a readable rule row to the Schedule section. Selecting that row reopens it for editing, and a separate trash-icon action with an accessible label removes it.
...
- Template readiness is derived rather than stored. A template is Ready only when it has at least one activity and its entire graph is complete, acyclic, traceable to project end, bounded, and jointly satisfiable with positive durations. An activity does not require its own rule when an incoming relationship from a project-end-connected activity gives it a finite latest placement. Incomplete, disconnected, or contradictory templates remain editable as Needs setup but cannot create projects.
- The template activity table shows both explicit rules and their inverse relationships so users can trace the graph from either activity. A dependent activity states that it cannot start or finish until its referenced activity is complete; the referenced activity states which named dependent cannot start or finish until it is complete.
...
- Dated activities use a staged **Schedule** section for project-relative and activity-relative rules, but remain explicitly dated rather than using template latest-valid placement. Adding or editing a rule preserves valid dates; otherwise it proposes the smallest duration-preserving shift that satisfies the combined rules. Removing a rule never moves dates. Every consequential date, project-boundary, or exception change is previewed and confirmed before one atomic save; completed and cross-project work remain fixed barriers.
...
- Mobile project tiles are shared by Overview and Projects. They contain project name/type, status, an endpoint-labelled elapsed project window, an in-window Today marker, a same-width activity-completion track, overdue and blocked counts, and the next upcoming activity.
...
- Primary desktop table content uses at least 14px text, dates/counts use tabular numerals, and truncated project descriptions have an accessible way to reveal the full text. Mobile activity cards retain compact type, assignment, status, date, and a prominent upper-right overdue indicator; priority remains in the desktop table, filters, and activity detail rather than the mobile tile.
- Activity editors use progressive disclosure to reduce initial length: Activity details opens initially, while Project timing, Schedule and notes, Team members, External links, and Prerequisites begin collapsed. Invalid sections open automatically and collapsed sections summarize configured timing, owners, links, and dependencies.
...
| --- | --- | --- | --- |
| unit-vitest | unit | `pnpm test` | `lib/planning/scheduling.test.ts`; `lib/planning/project-schedule.test.ts`; `lib/planning/project-tile.test.ts`; `lib/planning/project-reschedule.test.ts`; `lib/planning/overview-timeline.test.ts`; `components/projects/reschedule-preview-dialog.test.ts`; `lib/planning/project-template.test.ts`; `lib/planning/activity-schedule.test.ts` |
| prj06-database | database | `pnpm db:test` | `supabase/tests/prj06_project_rescheduling.sql` |
| activity-work-management-database | database | `pnpm db:test` | `supabase/tests/activity_work_management.sql` |
| template-database | database | `pnpm db:test` | `supabase/tests/project_templates.sql` |
| data-api-privileges-database | database | `pnpm db:test` | `supabase/tests/data_api_privileges.sql` |
...
| AUTH-02 | Universal authenticated access to shared MVP data | `lib/data/planning-data.ts` (`loadData`); `lib/supabase.ts` | RLS policies named `authenticated shared access` in `202608290001_initial_schema.sql`; explicit least-privilege Data API grants in `202609060004` and `202609060005` | Implemented | Transactional database coverage verifies shared authenticated project/activity reads, anonymous isolation, RPC boundaries, and denied direct aggregate writes. Role-based access is intentionally deferred. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations); Verification: `pnpm db:test`, `pnpm db:test` |
...
| NAV-02 | In-flow mobile header with complete navigation drawer | `components/shell/mobile-nav.tsx`; `app/extended.css` | Supabase sign-out | Implemented | The linked, tightly cropped ginkgo mark returns to Overview without clipping. The non-sticky header scrolls with the document, while its hamburger opens a route-aware drawer containing every primary destination and Sign out. Escape, backdrop dismissal, focus trapping/return, and background scroll locking are implemented. The former fixed bottom navigation and its visual-viewport dependency are removed from the rendered shell. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations) |
...
| OVR-02 | Week, Month, Quarter, Half-year, Year, and All Events ranges; Month default | `components/overview/overview.tsx` range state and range tabs; `lib/planning/overview-timeline.ts`; shared mobile `components/projects/project-tile.tsx` and `lib/planning/project-tile.ts` | None | Implemented | Month is the default on the desktop Gantt. Forward windows all begin today and end after 7 days or 1, 3, 6, or 12 calendar months; All Events uses the quarter-normalized full visible-project span. Quarter uses monthly labels with weekly subdivision lines; other views use day, week, month, or quarter divisions as specified. Shared mobile project tiles replace the Gantt with exactly aligned project-window and activity-completion tracks; the first uses endpoint date labels, elapsed fill, and a visible Today marker only inside the project window. Unit tests cover active, future, past, and one-day windows. | Design: [#experience-design](#experience-design), [#dashboard-and-timeline-behavior](#dashboard-and-timeline-behavior); Verification: `pnpm test` |
| OVR-03 | Draft and completed project visibility controls | `components/overview/overview.tsx` visibility state | Project status | Implemented | Mobile places the Draft and Completed controls on a dedicated row beneath the single-line portfolio title; desktop retains the inline timeline toolbar. Browser coverage verifies the mobile stacking. | Design: [#experience-design](#experience-design), [#dashboard-and-timeline-behavior](#dashboard-and-timeline-behavior) |
...
| PRJ-04 | Project detail summary, timeline before activity list, and quick activity editing | `components/projects/project-detail.tsx`; `components/projects/project-schedule.tsx`; `lib/planning/project-schedule.ts`; `components/projects/project-activity-list.tsx`; `components/activities/activity-panel.tsx`; `/projects/[id]` route | Projects, activities, dependencies | Implemented | The date-scaled timeline occupies the full workspace without a redundant Attention panel, includes the project row and dated end marker, maps activities to their actual dates, and extends for post-project activity finishes. A fixed 18px-per-day scale uses light daily rules and bold labelled Monday rules, so the same distance always represents the same duration and longer schedules scroll horizontally. Sticky opaque row labels mask bars at the timeline boundary during horizontal scrolling. The project activity list shares the global filters, opens an activity from the whole row without a redundant Edit affordance, and uses the shared non-breaking secondary Add activity action. Activity details open first in the editor while the other compact sections remain collapsed until requested or validation requires them. Dependency connectors are intentionally excluded. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations), [#timeline-hierarchy](#timeline-hierarchy), [#list-and-card-views](#list-and-card-views) |
...
| ID | Capability | Primary implementation | Database / persistence | Coverage | Verification / known gap | Traceability |
| --- | --- | --- | --- | --- | --- | --- |
| ACT-01 | Cross-project activity table with required columns | `components/activities/activities-list.tsx`; `/activities` route | Activities, projects, types, owners | Implemented | Desktop table content uses 14px text and tabular numerals. Mobile cards expose project, type, assignment, status, dates, and a prominent upper-right overdue state; priority remains available in the desktop table, filters, and activity detail without crowding the mobile tile. The read-only activity summary uses four equal cells on desktop and a balanced two-by-two grid on mobile. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations), [#list-and-card-views](#list-and-card-views) |
...
| ACT-03 | Create and edit required dates, assigned team members, type, status, priority, notes, and project | `components/activities/activity-form.tsx`; `components/shared/editor.tsx`; `components/activities/activity-panel.tsx`; `components/activities/activity-detail.tsx`; overview and project-detail launch points | Authoritative `save_activity` in `202608300003_project_timing_constraints.sql`; activities and `activity_owners` | Implemented | Shared editors use progressive disclosure, 16px controls, 44px actions, required markers, inline errors, linked summaries, ARIA associations, and first-invalid focus. Activity details opens initially; every secondary section begins collapsed even when configured, summarizes its values, and opens automatically when validation requires attention. Transactional database coverage verifies authoritative aggregate saves; browser CRUD coverage remains under QA-05. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations), [#list-and-card-views](#list-and-card-views); Verification: `pnpm db:test` |
| ACT-04 | Add multiple labeled external links through a staged modal and manage them as clickable, removable tiles | `lib/data/planning-data.ts`; `components/activities/link-editor.tsx`; `components/activities/activity-form.tsx`; `components/activities/activity-detail.tsx` | `activity_links`; authoritative `save_activity` in migration `202608300003` | Partial | Modal Save/Cancel, clickable tiles, explicit removal, and active-link filtering are implemented; transactional database coverage verifies link persistence through the authoritative aggregate save. Links render in the side panel rather than the specified bottom position. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations), [#list-and-card-views](#list-and-card-views); Verification: `pnpm db:test` |
...
| ACT-07 | Multiple assigned team members and unassigned activities | `components/activities/team-member-picker.tsx`; `components/shared/avatar-list.tsx`; overview metric/detail list and activity filters | `activity_owners`; team members | Implemented | Manual UI verifies multi-select staging and explicit removal; transactional database coverage verifies owner persistence through the authoritative activity save. Browser CRUD coverage remains under QA-05. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations), [#list-and-card-views](#list-and-card-views); Verification: `pnpm db:test` |

### Project templates

| ID | Capability | Primary implementation | Database / persistence | Coverage | Verification / known gap | Traceability |
| --- | --- | --- | --- | --- | --- | --- |
| TPL-01 | Create, search, edit, validate, and archive reusable project templates with deterministic project-end-connected activity graphs | Binding detail in `plans/TPL-01-project-templates.md`; `lib/planning/project-template.ts`; `components/templates/*`; `/templates` routes | `project_templates`; `project_template_activities`; `project_template_activity_rules`; authoritative saves, validation, and archive functions in migrations `202609060001`, `202609060002`, and `202609060003` | Partial | Positive inclusive durations, one-day defaults, multiple staged schedule rules, deterministic latest-valid resolution (including prerequisites derived through incoming relationships), bidirectional relationship summaries, read-only linked incoming rules in the activity editor, project-end traceability, cycle and contradiction rejection, responsive navigation, and inline referenced-activity creation are implemented. Unit tests and the 26-case local template database suite pass. Authorized desktop, 390×844, and physical-phone-origin review covered the blank state, add/edit/remove flow, conditional activity controls, nested creation, accessibility labels, responsive containment, compact list counts, wrapped schedule text, Chrome iOS hydration compatibility, and plain-HTTP LAN identifier fallback; automated data-changing browser coverage remains under QA-05. Migration `202609060003` is applied to the linked backend. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations); Plans: `plans/TPL-01-project-templates.md`; Verification: `pnpm test`, `pnpm db:test` |
| TPL-02 | Search valid templates, preview a schedule from project name and end date, and atomically create an independent dated Draft project and Not Started activity graph | Binding detail in `plans/TPL-01-project-templates.md`; `components/projects/project-create-flow.tsx`; `lib/planning/project-template.ts` | `preview_project_template`; `create_project_from_template` in migrations `202609060001`, `202609060002`, and `202609060003` | Partial | Searchable Ready-template selection, latest-valid multi-rule preview, independent atomic creation, inherited types, empty ownership/content fields, generated dependencies/timing rules, derived project start, and rollback on invalid graphs are implemented. Unit and local transaction suites pass; automated data-changing browser coverage remains under QA-05. | Design: [#experience-design](#experience-design), [#data-and-integrations](#data-and-integrations); Plans: `plans/TPL-01-project-templates.md`; Verification: `pnpm test`, `pnpm db:test` |
...
| DEP-04 | Prevent circular dependencies and explain the relationship path | Dependency candidate exclusion of self; database `prevent_dependency_cycle` trigger | `prevent_dependency_cycle` in initial schema | Partial | Transactional database coverage verifies cycle rejection and aggregate rollback. Selection-time path detection and full-path explanation are not implemented. | Design: [#data-and-integrations](#data-and-integrations), [#timeline-hierarchy](#timeline-hierarchy); Verification: `pnpm db:test` |
...
| DEP-09 | Activities may finish after project end with an explicit exception | `components/activities/activity-form.tsx` (`submit`) | `allow_outside_project`; authoritative activity functions and restricted table grants in `202609030002_protect_activity_exceptions.sql` | Implemented | The UI obtains confirmation for an unauthorized after-project finish. Transactional database tests verify consistent exception flags, denied direct authenticated writes, and authenticated/anonymous function boundaries. The migration preflight found no inconsistent existing flags. | Design: [#data-and-integrations](#data-and-integrations), [#timeline-hierarchy](#timeline-hierarchy); Verification: `pnpm db:test` |
| DEP-10 | Project start is on or before every active activity start; an earlier activity atomically proposes moving project start | `components/activities/activity-form.tsx`; `components/projects/project-form.tsx`; `lib/planning/scheduling.ts` (`movesProjectStart`) | `validate_activity_project_timing`; `validate_project_activity_timing`; `save_activity` in `202608300004_project_start_envelope.sql` | Implemented | Client and database enforcement are implemented; transactional database coverage verifies rejection and the authorized atomic project-start adjustment. Additional client boundary tests remain under QA-03. | Design: [#data-and-integrations](#data-and-integrations), [#timeline-hierarchy](#timeline-hierarchy); Verification: `pnpm db:test` |
| DEP-11 | Apply staged schedule rules to dated activities with preservation-first derivation and confirmed atomic propagation | Binding detail in `plans/DEP-11-dated-activity-schedule-rules.md`; Slice 1 placement kernel in `lib/planning/activity-schedule.ts`; later connected-graph planner and staged activity Schedule interface | Existing activity timing and dependency fields for Slice 1; authoritative preview/commit RPCs deferred to Slice 3 | Partial | Slice 1 deterministically preserves valid dates or applies the minimum duration-preserving shift against combined start/due bounds, including contradiction and calendar-boundary coverage. Connected-graph propagation, authoritative persistence, and the staged dated-activity interface remain planned. | Design: [#data-and-integrations](#data-and-integrations), [#timeline-hierarchy](#timeline-hierarchy); Plans: `plans/DEP-11-dated-activity-schedule-rules.md`; Verification: `pnpm test` |
...
| ARC-02 | Archive projects and their activities/links/dependencies | `components/projects/project-detail.tsx` (`archiveProject`) | `archive_project` | Implemented | Confirmation exists; transactional database coverage verifies retained project, activity, link, and cross-project dependency archive markers. No restore flow. | Design: [#experience-design](#experience-design), [#visual-direction](#visual-direction), [#data-and-integrations](#data-and-integrations); Verification: `pnpm db:test` |
| ARC-03 | Archive an activity with downstream-impact preview and archive affected relationships | No complete UI entry point | `archive_activity` in migration `202608300002` archives the activity, links, and relationships | Partial | Transactional database coverage verifies that the function retains and marks the activity, its links, and incoming and outgoing dependencies while leaving downstream activities active. The function is unreachable from the app; the activity archive control and required downstream-impact preview are missing. | Design: [#experience-design](#experience-design), [#visual-direction](#visual-direction), [#data-and-integrations](#data-and-integrations); Verification: `pnpm db:test` |
| ARC-04 | Retain archived dependency relationships for history while hiding them and excluding them from scheduling/cycle logic | `lib/data/planning-data.ts` filters active dependencies | `archive_activity`; `archive_project`; `prevent_dependency_cycle`; dependency `archived_at` | Implemented | Active scheduling and cycle traversal ignore archived relationships. Transactional database coverage verifies that activity and project archive operations retain dependency rows with archive markers. No visible history surface exists. | Design: [#experience-design](#experience-design), [#visual-direction](#visual-direction), [#data-and-integrations](#data-and-integrations); Verification: `pnpm db:test` |
...
| QA-03 | Automated unit tests for scheduling calculations | Vitest; `lib/planning/scheduling.test.ts`; `lib/planning/project-schedule.test.ts`; `lib/planning/project-tile.test.ts`; `lib/planning/project-reschedule.test.ts` | Partial | Project timing, project rescheduling, fixed daily project-schedule spacing and Monday marks, project-tile Today positioning for active/future/past/one-day windows, both dependency types, completed chronology, cross-project direction, exceptions, archive behavior, and calendar boundaries are covered. Add unit coverage for later scheduling features as they are implemented; transaction coverage remains under QA-04. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#technical-direction](#technical-direction); Verification: `pnpm test` |
| QA-04 | Database integration tests | PRJ-06 pgTAP suite in `supabase/tests/prj06_project_rescheduling.sql`; activity work-management pgTAP suite in `supabase/tests/activity_work_management.sql`; Data API privilege suite in `supabase/tests/data_api_privileges.sql`; disposable-Supabase `database` job in `.github/workflows/verify.yml`; explicit-barrier concurrency harness in `scripts/test-prj06-concurrency.sh`; transactional live verification migrations `202609040002`, `202609040005`, `202609040006`, and `202609040008` | Partial | The required pull-request workflow runs all reusable pgTAP suites against disposable Supabase. Activity coverage verifies authoritative aggregate saves, owners, links, dependencies, cycle rollback, dependency chronology, timing and exception boundaries, project-start adjustment, archive retention, shared authenticated reads, anonymous isolation, and denied direct writes. Data API coverage verifies least-privilege table and function grants, protected aggregate boundaries, administrative reference writes, service-role access, RLS retention, and secure defaults for future objects. PRJ-06 separately covers atomic rescheduling, stale fingerprints, and the complete scoped-lock concurrency matrix. Extend coverage to finer row-level authorization rules as those rules evolve. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#technical-direction](#technical-direction); Verification: `pnpm db:test`, `pnpm db:test`, `pnpm db:test` |
| QA-05 | End-to-end core-journey tests | Playwright smoke suite in `e2e/*`; authenticated read-only setup; mobile Chromium at 390 × 844; desktop Chromium at 1440 × 900; guarded dedicated-development environment and reserved fixture reset in `scripts/supabase-environment.mjs` and `scripts/seed-development.mjs` | Partial | Slice 1 covers responsive header/drawer navigation, project-editor team/action containment, accessible project validation, shared checkbox geometry, complete activity/project filter responsiveness, stacked mobile portfolio controls, shared aligned project-tile tracks and Today state, activity-card simplification, equal detail summaries, administration containment, fixed daily schedule spacing and boundary clipping, and mobile project-activity editor disclosure defaults plus label/section/action/notes containment. The hosted development backend and reserved resettable fixture graph now unblock isolated activity-form mutation, CRUD, dependency, and administration journeys; add those journeys with cleanup before making the suite required in CI. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#technical-direction](#technical-direction); Plans: `plans/QA-05-end-to-end.md`; Verification: `pnpm test:e2e` |
| QA-06 | Build and deployment target | `package.json`; `next.config.ts`; `.github/workflows/verify.yml`; Tailwind/PostCSS configuration; Supabase CLI configuration and migrations; guarded hosted-environment selection in `scripts/supabase-environment.mjs` | Implemented | Pull requests and main run repository and disposable-Supabase database verification in CI. Vercel deployment checks remain required before merge. Local application startup defaults to dedicated development and refuses a production-selected environment; hosted database writes require a matching app/CLI target and an explicit per-command authorization value. | Design: [#confirmed-project-constraints](#confirmed-project-constraints), [#technical-direction](#technical-direction) |
...
| 2026-08-29 | Use persistent sidebar navigation on desktop and primary bottom navigation on mobile. | Superseded on mobile by the 2026-09-05 document-flow header decision after browser viewport behavior proved unreliable. | Superseded |
...
| 2026-09-05 | Replace fixed mobile bottom navigation with a document-flow Threshold header and complete navigation drawer. | Removes dependence on Chrome’s unstable lower visual viewport while retaining all destinations; the header can scroll away and the linked brand provides a direct route to Overview. | Confirmed |
...
| 2026-09-05 | Use a fixed 18px-per-day project-detail schedule with light daily rules, bold labelled Mondays, and a dated project-end marker. | Keeps duration visually consistent on mobile and desktop while allowing long schedules to scroll instead of compressing early periods. | Confirmed |
| 2026-09-05 | Align project-window elapsed time and activity completion as equal-width tracks on shared mobile project tiles. | Makes schedule progress and completed work directly comparable without replacing manually selected project health; Today is visibly marked only inside the project window. | Confirmed |
| 2026-09-06 | Give template activities a positive inclusive duration defaulting to one day, allow multiple simultaneous scheduling constraints, and resolve the latest valid project-end-connected schedule. | Makes same-day work intuitive, defines duration once per activity, and supports realistic combinations of project deadlines and prerequisite relationships while rejecting cycles, unbounded graphs, and contradictory constraints. | Confirmed |
...
| D-15 | Navigation | What navigation model best fits the management team's working style? | Confirmed | Persistent desktop sidebar; non-sticky mobile Threshold header with a complete hamburger navigation drawer. |
...
- 2026-09-05: Reconciled the Instinct product review with confirmed Threshold behavior; implemented responsive, validation, progressive-disclosure, empty-state, legibility, contrast, and Today-marker improvements. The initial bottom-navigation retention was subsequently superseded by the confirmed document-flow mobile header and drawer.
