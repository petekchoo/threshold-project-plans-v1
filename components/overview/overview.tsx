...
export function Overview({ data, refresh, setToast }: { data: AppData; refresh: () => void; setToast: (value: string) => void }) {
  ...
  const activityCount = (count: number, plural: string) => `${count} ${count === 1 ? 'activity' : plural}`;

  return <><Heading eyebrow={new Intl.DateTimeFormat('en-US',{weekday:'long',month:'long',day:'numeric'}).format(new Date())} title="Threshold at a glance" copy="What is moving, what needs attention, and what comes next."/><section className="metric-grid" aria-label="Activity attention summary"><a href="#overdue-activities"><article className="metric-tile metric-overdue"><strong>{overdue.length}</strong><span>{activityCount(overdue.length, 'activities').replace(/^\d+ /, '')} overdue</span><small>Incomplete work past its due date</small></article></a><a href="#due-soon-activities"><article className="metric-tile metric-due"><strong>{dueSoon.length}</strong><span>{activityCount(dueSoon.length, 'activities').replace(/^\d+ /, '')} due in 7 days</span><small>Upcoming work that is not overdue</small></article></a><a href="#unassigned-activities"><article className="metric-tile metric-unassigned"><strong>{unassigned.length}</strong><span>Unassigned {activityCount(unassigned.length, 'activities').replace(/^\d+ /, '')}</span><small>Work ready for a team member</small></article></a></section><section className="portfolio-card"><div className="section-head"><h2>Active projects</h2><div className="timeline-visibility" aria-label="Project visibility filters"><span>Show</span><label><input type="checkbox" checked={showDraft} onChange={event=>setShowDraft(event.target.checked)}/> Draft</label><label><input type="checkbox" checked={showCompleted} onChange={event=>setShowCompleted(event.target.checked)}/> Completed</label><Link href="/projects">View all projects →</Link></div></div><div className="range-tabs" aria-label="Portfolio timeline range">{RANGE_OPTIONS.map(option=><button type="button" aria-pressed={range===option} className={range===option?'selected':''} onClick={()=>setRange(option)} key={option}>{option}</button>)}</div><div className="gantt desktop-only"><div className="gantt-header"><div>Project</div><div className="overview-scale" role="img" aria-label={scaleLabel}>{gridLines}{scale.marks.map(mark=><span aria-hidden="true" key={mark.date} style={{left:`${mark.left}%`,width:`${mark.width}%`}}>{mark.label}</span>)}</div></div>{visible.map(project=>{const position=overviewBarPosition(project.start_date,project.end_date,scale.domainStart,scale.domainEnd);return <div className="gantt-row" key={project.id}><Link className="project-label" href={`/projects/${project.id}`}><strong>{project.name}</strong><span>{project.project_types?.name} · {progress(project,data.activities)}% complete</span></Link><div className="timeline overview-timeline">{gridLines}{position&&<div className={`bar status-${project.status} ${position.extendsBefore?'bar-extends-before':''} ${position.extendsAfter?'bar-extends-after':''}`} style={{left:position.left,width:position.width}}><span>{statusLabel(project.status)}</span><b style={{width:`${progress(project,data.activities)}%`}}/></div>}</div></div>})}</div><div className="project-cards mobile-only">{visible.map(project=><ProjectCard key={project.id} p={project} activities={data.activities}/>)}</div></section><div className="overview-attention-grid"><ActivityAttentionList id="overdue-activities" title="Overdue activities" empty="No incomplete activities are overdue." activities={overdue} onEdit={setEditingActivity}/><ActivityAttentionList id="due-soon-activities" title="Due in the next 7 days" empty="No incomplete activities are due in the next seven days." activities={dueSoon} onEdit={setEditingActivity}/><ActivityAttentionList id="unassigned-activities" title="Unassigned activities" empty="Every activity has a team member." activities={unassigned} onEdit={setEditingActivity}/></div><OwnerBreakdown data={data}/>{editingActivity&&<ActivityPanel activity={editingActivity} data={data} onClose={()=>setEditingActivity(null)} onSaved={()=>{setEditingActivity(null);refresh();setToast('Activity changes saved.')}}/>}</>;
}
