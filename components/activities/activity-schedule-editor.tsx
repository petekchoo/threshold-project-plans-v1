...
export function ActivityScheduleEditor({ initial, data, name, projectId, startDate, dueDate, timingRule, timingOffsetDays, dependencies,
  ...
  function candidateState(draft:DraftRule){
    ...
    if(relationshipRule(draft.schedule_rule)){
      const dependency:Dependency={depends_on_activity_id:draft.relative_activity_id,constraint_type:draft.schedule_rule==='start_after_activity_finish'?'finish_to_start':'finish_to_finish',offset_days:draft.offset_days};
