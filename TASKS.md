...
- Complete the phased mobile UX follow-up. Phase 1 contains project-list controls, removes desktop-only portfolio range controls from the mobile tile view, and corrects compressed project-schedule labels. Phase 2 establishes shared typography, button, control, checkbox, focus, and action-icon treatments; contains narrow-pane summary-pane long content; refines the mobile portfolio header, scrolling schedule boundary, Add activity action, full-width Notes field, required markers, nested section headings, and Chrome-obscured editor actions; defaults secondary activity-editor sections closed; and gives the shared project schedule a fixed daily scale with Monday labels. Phase 3 replaces the unstable fixed bottom navigation with a non-sticky mobile header whose cropped ginkgo mark links to Overview and whose accessible drawer contains every primary destination and Sign out; aligns Projects with the responsive activity filter pattern; equalizes the activity detail summary; simplifies mobile activity tiles by elevating overdue state and omitting priority; removes provisional overview corner accents; aligns overview activity-name typography; and introduces shared project tiles with equal-width elapsed-window and activity-completion tracks plus an in-window Today marker. Commit `9cd0e87` remains the exact clean-production reference for the superseded Chrome workaround. LAN development hydration required an explicit `allowedDevOrigins` configuration. Preserve Safari, Edge, and desktop behavior and carry the local post-PR #25 handoff commit through this feature branch rather than pushing it directly to `origin/main`.
- Pull request #26 contains the complete mobile UX follow-up and is pending merge. On head `59f56be`, GitHub Verify run 60 and the Vercel deployment check passed; rerun both gates for the final documentation-only head before merging.
